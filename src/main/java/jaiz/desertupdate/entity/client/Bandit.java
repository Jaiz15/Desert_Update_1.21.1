...
public class Bandit <T extends BanditEntity> extends SinglePartEntityModel<T> {
	...
	public void setAngles(BanditEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		this.animateMovement(ModAnimations.BANDIT_WALK, limbSwing, limbSwingAmount, 2f, 2.5f);
		this.updateAnimation(entity.idleAnimationState, ModAnimations.BANDIT_IDLE, ageInTicks, 1f);
		this.updateAnimation(entity.attackAnimationState, ModAnimations.BANDIT_ATTACK, ageInTicks, 1f);
	}
